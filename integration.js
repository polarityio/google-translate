...
function doLookup(entities, options, cb) {
    ...
    async.parallelLimit(tasks, 10, (err, results) => {
        ...
        results.forEach(result => {
            Logger.trace({result: result}, "Checking data to see if blocking");
            ...
            }else if(result.body.data.translations){
              let englishCheck = result.body.data.translations;
              englishCheck.forEach(function(english){
                Logger.trace({result: english.detectedSourceLanguage}, "Checking if this works");
                if (english.detectedSourceLanguage === "en"){
                  lookupResults.push({
                      entity: result.entity,
                      data: null
                  });
